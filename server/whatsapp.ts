...
function getContactName(jid: string): string {
  ...
  // Tenta obter o nome do contato da lista de contatos do Baileys
  try {
    ...
    // se o store não estiver disponível ainda
  }
...
function upsertConversation(jid: string, msg: StoredMessage, pushName?: string) {
  ...
  if (!conversations.has(jid)) {
    // Para novas conversas, tenta usar o pushName (vindo de mensagens recebidas)
    const initialName = (!msg.fromMe && pushName) ? pushName : getContactName(jid);
    ...
    // Só atualiza o nome com pushName se a mensagem NÃO for do usuário
    // Isso evita que o nome do usuário (Eduardo Coelho) sobrescreva o nome do contato
    if (!msg.fromMe && pushName && pushName !== conv.name) {
      conv.name = pushName;
    }
    // Também tenta atualizar com o nome do store de contatos
    const betterName = getContactName(jid);
    if (betterName !== conv.name && betterName !== phoneDisplay(phone)) {
...
export async function connectWhatsApp(): Promise<void> {
  ...
  // ── Atualiza nomes dos contatos quando a lista de contatos é sincronizada ──
  sock.ev.on('contacts.upsert', (contacts) => {
    console.log(`👥 ${contacts.length} contato(s) atualizado(s)`);
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    // 'notify' = mensagem nova; 'append' = histórico sincronizado
    if (type !== 'notify' && type !== 'append') return;

    console.log(`📨 ${type}: ${messages.length} mensagem(ns) recebida(s)`);
    ...
    for (const raw of messages) {
      ...
      if (!body) {
        console.log(`  ↳ Mensagem ignorada (sem corpo): ${raw.key.id}`);
        continue;
      } // Ignora eventos silenciosos
